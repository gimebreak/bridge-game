...
class GameUserOnline(models.Model):
    _inherit='res.users'

    # @api.depends('im_status')
    @api.multi
    def send_imstatus(self,channel_id):
        my_imstatus = self.env.user.im_status
        name = self.env.user.partner_id.name
        vals = {'name':name,'status':my_imstatus}
        self.env['og.channel'].browse(channel_id).message_post(body = my_imstatus)
        return vals
