...
#warning: a table object may relate to many channels
    ...
    def is_playing(self,table_id):
        ...
        blank = self.env['og.board']
        for board in boards:

            if board.result2 !=None:
                tmp_board += board

            if board.call_ids.mapped('id') != [] and board.result2 == False:
...
# able to return more infomations
    ...
    def reg_test(self):
        ...
        for m in message_set:
            try:context = re.search('^<p>(.*)</p>$',m.body)
            except:continue
            try:context['pos']
            except KeyError:
                continue
