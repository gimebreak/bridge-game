...
class Board(models.Model):
    ...
    testing = fields.Char(compute='test_compute')
    ...
    def test_compute(self):
        for rec in self:
            table = rec.table_id
            channel = self.env['og.channel'].search([('table_id','=',table.id)])
            bodyinfo = self.name
            # self.env['og.channel'].browse(channel.id).message_post(body=bodyinfo)
            rec.testing = bodyinfo
...
#teting
    ...
    def tested(self):
        ...
        res = [ cs.filtered(lambda c: c.trickno==t) for t in ts]
        return res[0]
