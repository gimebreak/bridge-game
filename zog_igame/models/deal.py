...
class Deal(models.Model):
    ...
    ad_pos = fields.Selection(POSITIONS,compute = '_ad_pos')
    @api.multi
    def _ad_pos(self):
        positions = 'NESW'
        for rec in self:
            all_points = []
            dealname = rec.name
            all_deals= dealname.split()

            for deal in all_deals:
                points = self._check_goodcard(deal)
                all_points.append(points)

            max_point = max(all_points)
            max_pos = all_points.index(max_point)
            rec.ad_pos = positions[max_pos]


    def _check_goodcard(self,deal):
        deal_count = 'JQKA'
        total_points = 0
        for card in deal:
            if card in deal_count:
                card_point = deal_count.index(card)+1
